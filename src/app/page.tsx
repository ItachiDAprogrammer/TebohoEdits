...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const onSelect = () => {
      setSelectedIndexReels(emblaApiReels.selectedScrollSnap())
    }
    ...
    return () => {
      emblaApiReels.off('select', onSelect)
    }
  }, [emblaApiReels])
  ...
  useEffect(() => {
    ...
    const onSelect = () => {
      setSelectedIndexClients(emblaApiClients.selectedScrollSnap())
    }
    ...
    return () => {
      emblaApiClients.off('select', onSelect)
    }
  }, [emblaApiClients])
    ...
    <div className="min-h-screen flex flex-col bg-background">
            ...
            <Card className="border-[#E50914]/20 hover:border-[#E50914]/50 transition-all hover:shadow-lg hover:shadow-[#E50914]/10">
              ...
              <CardContent>
                <div className="flex flex-wrap gap-1 md:gap-2">
                  <Badge className="text-[10px] md:text-xs bg-[#E50914]/10 text-[#E50914] border-[#E50914]/20">Filmora</Badge>
                  <Badge className="text-[10px] md:text-xs bg-[#E50914]/10 text-[#E50914] border-[#E50914]/20">CapCut</Badge>
                  <Badge className="text-[10px] md:text-xs bg-[#E50914]/10 text-[#E50914] border-[#E50914]/20">DaVinci Resolve</Badge>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-[#E50914]/20 hover:border-[#E50914]/50 transition-all hover:shadow-lg hover:shadow-[#E50914]/10">
              ...
              <CardContent>
                <ul className="space-y-1 md:space-y-2 text-xs md:text-sm text-muted-foreground">
                  <li className="flex items-center gap-2"><CheckCircle className="w-3 h-3 md:w-4 md:h-4 text-[#E50914]" />Narrative-driven editing</li>
                  <li className="flex items-center gap-2"><CheckCircle className="w-3 h-3 md:w-4 md:h-4 text-[#E50914]" />Color grading</li>
                  <li className="flex items-center gap-2"><CheckCircle className="w-3 h-3 md:w-4 md:h-4 text-[#E50914]" />Motion graphics</li>
                  <li className="flex items-center gap-2"><CheckCircle className="w-3 h-3 md:w-4 md:h-4 text-[#E50914]" />Sound design</li>
                </ul>
              </CardContent>
            </Card>
      ...
      <section id="long-form" className="py-12 md:py-16 px-4 sm:px-6 md:px-8 lg:px-16 bg-gradient-to-b from-[#E50914]/5 to-transparent">
        <div className="max-w-7xl mx-auto space-y-8 md:space-y-10">
          <div className="text-center space-y-3 md:space-y-4">
            <h2 className="text-3xl md:text-5xl font-bold font-['Batman_Forever'] text-[#E50914]">Long Form</h2>
            <p className="text-sm md:text-base text-muted-foreground max-w-2xl mx-auto font-['Arial']">Full-length, horizontal videos showcasing cinematic storytelling</p>
          </div>
          ...
          {longVideos.length === 0 ? (
            <Card className="text-center py-10 md:py-12">
              <CardContent>
                ...
                <p className="text-sm md:text-base text-muted-foreground">No long-form videos yet. Check back soon!</p>
              </CardContent>
            </Card>
          ) : (
      ...
      <section id="reels" className="py-12 md:py-16 px-4 sm:px-6 md:px-8 lg:px-16">
        <div className="max-w-7xl mx-auto space-y-8 md:space-y-10">
          <div className="text-center space-y-3 md:space-y-4">
            <h2 className="text-3xl md:text-5xl font-bold font-['Batman_Forever'] text-[#E50914]">Reels</h2>
            <p className="text-sm md:text-base text-muted-foreground max-w-2xl mx-auto font-['Arial']">Short-form, vertical content optimized for social media</p>
          </div>
          ...
          {shortVideos.length === 0 ? (
            <Card className="text-center py-10 md:py-12">
              <CardContent>
                <Youtube className="w-12 h-12 md:w-16 md:h-16 mx-auto text-muted-foreground mb-4" />
                <p className="text-sm md:text-base text-muted-foreground">No reels yet. Check back soon!</p>
              </CardContent>
            </Card>
          ) : (
            <div className="relative">
              <div className="overflow-hidden" ref={emblaRefReels}>
                <div className="flex gap-4 md:gap-4">
                  {/* Left spacer for permanent padding */}
                  <div className="flex-shrink-0 w-0.5 md:w-6" aria-hidden="true" />
                  {shortVideos.map((video) => (
                    <div key={video.id} className="w-[160px] sm:w-[200px] md:w-[240px] lg:w-[260px] flex-shrink-0">
                      <VideoCard video={video} onPlay={() => { setIsVideoLoading(true); setSelectedVideo(video); }} />
                    </div>
                  ))}
                  {/* Right spacer for permanent padding */}
                  <div className="flex-shrink-0 w-0.5 md:w-6" aria-hidden="true" />
                </div>
              </div>
              ...
              <div className="flex justify-center mt-4 gap-2">
                {shortVideos.map((_, index) => (
                  <button
                    key={index}
                    onClick={() => emblaApiReels && emblaApiReels.scrollTo(index)}
                    className={`w-2 h-2 rounded-full ${index === selectedIndexReels ? "bg-[#E50914]" : "bg-gray-400"}`}
                  />
                ))}
              </div>
            </div>
      ...
      <section className="py-12 md:py-16 px-4 sm:px-6 md:px-8 lg:px-16">
        <div className="max-w-7xl mx-auto space-y-8 md:space-y-10">
          <div className="text-center space-y-3 md:space-y-4">
            <h2 className="text-2xl md:text-4xl font-bold font-['Arial']">Services Offered</h2>
            <p className="text-sm md:text-base text-muted-foreground max-w-2xl mx-auto font-['Arial']">Comprehensive video production services tailored to your needs</p>
          </div>
      ...
      {certificates.length > 0 && (
        <section className="py-12 md:py-16 px-4 sm:px-6 md:px-8 lg:px-16 bg-gradient-to-b from-transparent to-[#E50914]/5">
          <div className="max-w-5xl mx-auto space-y-8 md:space-y-10">
            <div className="text-center space-y-3 md:space-y-4">
              <h2 className="text-2xl md:text-4xl font-bold font-['Arial']">Certifications</h2>
              <p className="text-sm md:text-base text-muted-foreground font-['Arial']">Verified skills backed by formal training</p>
            </div>
                ...
                <Card key={cert._id} onClick={() => setActiveCertificate(cert)} className="cursor-pointer border-[#E50914]/20 hover:border-[#E50914]/50 transition-all hover:shadow-lg hover:shadow-[#E50914]/10">
                  <CardContent className="pt-4 md:pt-6 space-y-3 md:space-y-4">
                    ...
                    <div className="space-y-1">
                      <h3 className="text-sm md:text-lg font-semibold font-['Arial']">{cert.title}</h3>
                      {cert.issuer && <p className="text-xs md:text-sm text-muted-foreground font-['Arial']">Issued by {cert.issuer}</p>}
                    </div>
      ...
      {clients.length > 0 && (
        <section className="py-12 md:py-16 px-4 sm:px-6 md:px-8 lg:px-16 bg-gradient-to-b from-transparent to-[#E50914]/5">
          <div className="max-w-7xl mx-auto space-y-10 md:space-y-12">
            <div className="text-center space-y-3 md:space-y-4">
              <h2 className="text-2xl md:text-4xl font-bold font-['Arial']">Clients I've Worked With</h2>
              <p className="text-sm md:text-base text-muted-foreground max-w-2xl mx-auto font-['Arial']">Proud to have collaborated with these amazing creators and brands</p>
            </div>
            <div className="relative">
                      ...
                      <div className="relative">
                        <div className="w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 rounded-full overflow-hidden border-4 border-[#E50914]/30 bg-card">
                          {client.logo ? (
                            ...
                            <div className="w-full h-full bg-gradient-to-br from-[#E50914] to-[#C41110] flex items-center justify-center text-white font-bold font-['Arial'] text-lg md:text-2xl">{client.name.charAt(0)}</div>
                          )}
                        </div>
                      </div>
              ...
              <div className="flex justify-center mt-4 gap-2">
                {clients.map((_, index) => (
                  <button
                    key={index}
                    onClick={() => emblaApiClients && emblaApiClients.scrollTo(index)}
                    className={`w-2 h-2 rounded-full ${index === selectedIndexClients ? "bg-[#E50914]" : "bg-gray-400"}`}
                  />
                ))}
              </div>
            </div>
      ...
      <section id="contact" className="py-16 px-4 md:px-8 lg:px-16">
        <div className="max-w-3xl mx-auto space-y-8">
          <div className="text-center space-y-4">
            ...
            <p className="text-muted-foreground font-['Arial']">Ready to bring your vision to life? Get in touch and let's create something amazing.</p>
          </div>
          <Card className="border-[#E50914]/20">
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><Mail className="w-5 h-5 text-[#E50914]" />Send Me a Message</CardTitle>
              <CardDescription>Or email me directly at <a href="mailto:tebohoentene@gmail.com" className="text-[#E50914] hover:text-[#C41110]">tebohoentene@gmail.com</a></CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleContactSubmit} className="space-y-4">
                <div className="space-y-2">
                  <label htmlFor="name" className="text-sm font-medium font-['Arial']">Name</label>
                  <Input id="name" placeholder="Your name" value={contactForm.name} onChange={(e) => setContactForm({...contactForm, name: e.target.value})} required />
                </div>
                <div className="space-y-2">
                  <label htmlFor="email" className="text-sm font-medium font-['Arial']">Email</label>
                  <Input id="email" type="email" placeholder="your@email.com" value={contactForm.email} onChange={(e) => setContactForm({...contactForm, email: e.target.value})} required />
                </div>
                <div className="space-y-2">
                  <label htmlFor="message" className="text-sm font-medium font-['Arial']">Message</label>
                  <Textarea id="message" placeholder="Tell me about your project..." rows={5} value={contactForm.message} onChange={(e) => setContactForm({...contactForm, message: e.target.value})} required />
                </div>
                <Button type="submit" className="w-full bg-[#E50914] hover:bg-[#C41110] text-white font-['Arial']" disabled={formStatus === 'loading'}>
      ...
      <Dialog open={!!activeCertificate} onOpenChange={(open) => !open && setActiveCertificate(null)}>
        <DialogContent className="!p-0 w-[95vw] lg:w-[90vw] max-w-[1600px] bg-background border-[#E50914]/20 overflow-hidden flex flex-col">
          {activeCertificate && (
            <div className="space-y-4 flex flex-col items-center">
              <DialogHeader className="text-center">
                <DialogTitle className="font-['Arial'] text-xl md:text-2xl">{activeCertificate.title}</DialogTitle>
                {activeCertificate.issuer && <DialogDescription className="font-['Arial'] text-sm md:text-base">Issued by {activeCertificate.issuer}</DialogDescription>}
              </DialogHeader>
              <div className="relative w-full flex-1 max-h-[70vh]">
                <img src={activeCertificate.imageUrl} alt={activeCertificate.title} className="w-full h-full object-contain rounded-md" />
              </div>
            </div>
          )}
        </DialogContent>
      ...
      <Dialog open={!!selectedVideo} onOpenChange={(open) => !open && setSelectedVideo(null)}>
        <DialogContent className="!p-0 !w-screen !max-w-none !h-screen bg-background border-none overflow-hidden flex flex-col">
          <div className="flex items-center justify-between p-4 border-b border-[#E50914]/20 bg-[#E50914]/5">
            <div className="flex-1">
              ...
              <DialogDescription className="text-muted-foreground font-['Arial']">{selectedVideo?.category === 'long' ? 'Long Form' : 'Short Form'} Video</DialogDescription>
            </div>
            <Button variant="ghost" size="icon" onClick={() => setSelectedVideo(null)} className="text-foreground hover:text-[#E50914]"><X className="w-6 h-6" /></Button>
          </div>
              ...
              selectedVideo.category === 'short' ? (
                <div className="h-full flex items-center justify-center">
                  <div className="h-[85vh] aspect-[9/16]">
                    <iframe className="w-full h-full" src={`https://www.youtube.com/embed/${selectedVideo.youtubeId}?autoplay=1&rel=0`} title={selectedVideo.title} onLoad={() => setIsVideoLoading(false)} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen />
                  </div>
                </div>
              ) : (
          ...
          {selectedVideo?.description && (
            <div className="p-4 border-t border-[#E50914]/20">
              <p className="text-sm text-muted-foreground font-['Arial']">{selectedVideo.description}</p>
              <a href={`https://www.youtube.com/watch?v=${selectedVideo.youtubeId}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 mt-4 text-sm text-[#E50914] hover:text-[#C41110] font-semibold font-['Arial']">View on YouTube <ChevronRight className="w-4 h-4" /></a>
            </div>
          )}
        </DialogContent>
      </Dialog>
